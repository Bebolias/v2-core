...
function mulUDxUint(UD60x18 a, uint256 b) view returns (uint256) {
  return UD60x18.unwrap(mulUD60x18(a, UD60x18.wrap(b)));
}

function mulUDxInt(UD60x18 a, int256 b) view returns (int256) {
  return mulSDxUint(SD59x18.wrap(b), UD60x18.unwrap(a));
}

function mulSDxInt(SD59x18 a, int256 b) view returns (int256) {
  return SD59x18.unwrap(mulSD59x18(a, SD59x18.wrap(b)));
}

function mulSDxUint(SD59x18 a, uint256 b) view returns (int256) {

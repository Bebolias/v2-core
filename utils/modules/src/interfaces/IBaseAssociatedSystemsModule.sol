 ...
 * - Managed systems are connected via a proxy, which means that their implementation can be updated, and the system controls the execution context of the associated system. Example, account token connected to the system, and controlled by the system.
 * - Unmanaged systems are just addresses tracked by the system, for which it has no control whatsoever. Currently, we're not using these, however may consider using for interactions with external instruments and exchanges (e.g. dated irs instrument).
 *

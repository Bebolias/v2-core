 ...
 * Associated systems become available to all system modules for communication and interaction, but as opposed to inter-modular
 * communications, interactions with associated systems will require the use of `CALL`.
 *
 * Associated systems can be managed or unmanaged.
 * - Managed systems are connected via a proxy, which means that their implementation can be updated, and the system controls the
 * execution context of the associated system. Example, account token connected to the system, and controlled by the system.
 * - Unmanaged systems are just addresses tracked by the system, for which it has no control whatsoever. Currently, we're not using
 * these, however may consider using for interactions with external instruments and exchanges (e.g. dated irs instrument).
 *
...
interface IBaseAssociatedSystemsModule {
     ...
     * @param id The bytes32 identifier of the associated system. If the id is new to the system, it will create a new proxy for the
     * associated system.

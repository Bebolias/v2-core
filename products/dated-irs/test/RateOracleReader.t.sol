...
contract RateOracleReaderTest is Test {
    ...
    function test_updateCache_afterMaturity_noCache() public {
        ...
        assertEq(unwrap(index), indexToSet);
    }

    function test_updateCache_afterMaturity_zeroCache() public {
        ...
        assertEq(unwrap(index), indexToSet/2);
    }

    function test_updateCache_beforeMaturity_noCache() public {
    ...
    function test_updateCache_beforeMaturity_zeroCache() public {
        // update cache with index 0
    ...
    function test_fail_updateCache_afterMaturity_zeroCache() public {
        vm.warp(maturityTimestamp + 1);
        vm.expectRevert();
        UD60x18 index  = rateOracleReader.getRateIndexCurrent(marketId, maturityTimestamp);
    }

    function test_getRateIndexCurrent_afterMaturity_zeroCacheAtMaturity() public {
    ...
    function test_getRateIndexCurrent_afterMaturity_cacheAtMaturity() public {
        // can also test expectCall() to rate oracle
        vm.warp(maturityTimestamp + 1);
        ...
        assertEq(unwrap(index), indexToSet0);
    }

    function test_fail_getRateIndexMaturity_beforeMaturity() public {
        vm.expectRevert();
        UD60x18 index  = rateOracleReader.getRateIndexMaturity(marketId, maturityTimestamp);
    }

    function test_getRateIndexMaturity_afterMaturity() public {
        vm.warp(maturityTimestamp + 1);
